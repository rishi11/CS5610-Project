...
$(document).ready(function () {
    ...
    function ready() {
        ...
        $(".profilePage").click(openProfilePageHandler);
        $(".playSpeech").hide();
        $(".imageContent").hide();
    ...
    function saveWidgetHAndler() {
        ...
        widgets.each(function (index, widget) {
            ...
            if ((widget.hasClass('dashBoardGoogler')) && (!(widget.hasClass('cloneWidget')))) {
                ...
                } else if (widget.hasClass('dashBoardMesseneger') && (!(widget.hasClass('cloneWidget')))) {
                    console.log("Parse a messenger");
                    var position = widget.position();
                    var top = position.top;
                    var left = position.left;
                    var width = widget.outerWidth();
        ...
        $.ajax({
            dataType: "json",
            url: "http://localhost:3000/dashboardSave/" + sendObjJson +"/" + username,
    ...
    function loadWidgetHandler() {
        ...
        $.ajax({
            dataType: "json",
            url: "http://localhost:3000/dashboardGet/" + username,
            success: function (data) {
                displayWidgets(data);
    ...
    function displayWidgets(data) {
        ...
        for (var i = 0 ; i < widgetsrecieved.length; i++) {
            ...
            if (widgetsrecieved[i].type == "STICKY") {
                ...
                console.log("recived ff");
                var createWidget = $('.cloneFriendFinder').clone();
                console.log(createWidget);
                createWidget.removeClass('cloneFriendFinder');
                createWidget.removeClass('cloneWidget');
                createWidget.addClass('fresh');
                createWidget.draggable();
                createWidget.css({ position: "absolute", top: widgetsrecieved[i].top, left: widgetsrecieved[i].left, width: widgetsrecieved[i].width, height: widgetsrecieved[i].height });
                ...
                var dom = $(cloneFrom)
                    .css({ position: "absolute", top: widgetsrecieved[i].top, left: widgetsrecieved[i].width, width : widgetsrecieved[i].width , height : widgetsrecieved[i].height });
                $('#launchpad').append(dom);
            }
        }
    ...
    function getEventHandlerJquery() {
        ...
        if (username != null || username != "") {
            $.ajax({
                dataType: "json",
                url: "http://localhost:3000/geteventlist/" + username,
                success: function (data) {
                    console.log(data);
    ...
    function getAllUsers() {
        ...
        $.ajax({
            dataType: "json",
            url: "http://localhost:3000/getallusernames/" + username,
            success: function (data) {
                console.log(data);
                console.log(data.allOtherUserList);
                for (var i = 0 ; i < data.allOtherUserList.length; i++) {
                    console.log(data.allOtherUserList[i]);
                    $(".friendListTable > tbody").append('<tr><td><a target="_blank" href="otherProfile.html?user=' + username + '&profileUser=' + data.allOtherUserList[i] + '">' + data.allOtherUserList[i] + '<img class="moreicon" src="../images/more.png" /></a></td></tr>')
                }
            }
        });
        }
...
function addEventHandler() {
    ...
    if (todaysDate <= eventsDate) {
        if (!((eventName == "") || (eventName == ""))) {
            ...
            $.ajax({
                dataType: "json",
                url: "http://localhost:3000/eventStore/" + dateString + "/" + eventName + "/" + username,
                success: function (data) {
                    //console.log(data);
...
function getAllMessages() {
    ...
    if((username != "")||(username == null)){
        $.ajax({
            dataType: "json",
            url: "http://localhost:3000/getofflinemessages/" + username,
            success: function (data) {
                console.log(data);
                if (data != null) {
                    console.log("In if");
...
function sendReply(obj) {
    ...
    $.ajax({
        dataType: "json",
        url: "http://localhost:3000/sendofflinemsg/" + username + "/" + msgDetails + "/" + replyContent,
        success: function (data) {
            if (data.message == "success") {
