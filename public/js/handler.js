...
$(document).ready(function () {
    ...
    function addClock() {
        ...
        if ($(".hiddenClock").hasClass("hideClockMode")) {
            //console.log("Yes");
            $(".hiddenClock").draggable();
            $(".hiddenClock").show();
            $(".hiddenClock").removeClass("hideClockMode");
            $(".hiddenClock").css({ position: "absolute", top: 100, left: 100 });
        }
        $(".addWidgetCover").hide();
    }
    ...
    function loadWidgetHandler() {
        ...
        $.ajax({
            ...
            success: function (data) {
                //console.log(data);
                if (data.message == "initial store not there") {
                    //console.log("Return");
                    return;
                } else {
                    //console.log("Display");
    ...
    function displayWidgets(data) {
        ...
        for (var i = 0 ; i < widgetsrecieved.length; i++) {
            ...
            if (widgetsrecieved[i].type == "STICKY") {
                ...
                createWidget.addClass('fresh');
                createWidget.draggable();
                $(createWidget).find('.timeCountry').val(widgetsrecieved[i].atr1);
                createWidget.css({ position: "absolute", top: widgetsrecieved[i].top, left: widgetsrecieved[i].left, width: widgetsrecieved[i].width, height: widgetsrecieved[i].height });
                ...
                */
                ...
                $(".hiddenClock").show();
                $(".hiddenClock").removeClass("hideClockMode");
                $(".hiddenClock").css({ position: "absolute", top: widgetsrecieved[i].top, left: widgetsrecieved[i].left });
                $(".timeCountry").val(widgetsrecieved[i].atr1);
                loadClock(widgetsrecieved[i].atr1);
    ...
    function getWeatherJquery(obj, atr1, atr2) {
        ...
        (obj).find('.zipCode').val(atr1);
        (obj).find('.country').val(atr2);
        ////console.log("Optins set : ")
...
function deleteWidgetAction(obj) {
    if ($(obj).hasClass('deleteCalendar')) {
        ...
        $(".hiddenClock").addClass('hideClockMode');
        $(".hiddenClock").hide();
